...
import { setEditpackagesidebar } from 'redux/reducers/MainmenuReducer'

import { setModalMessage, setModalToast } from 'redux/reducers/ModalReducer'
//---- REDUX STORE ---------------------
import { useSelector, useDispatch } from 'react-redux'
import {setAdminPackageArray, setAllowRelaodPAckage, itemToEdit } from 'redux/reducers/PackageReducer'
...
export default function Home() {
 ...
 const dispatch = useDispatch()
  ...
  const { adminPackageArray, allowRelaodPackage, itemToEdit } = useSelector((state) => state.PackageReducer)
  ...
  useEffect(() => {
    if (itemToEdit) {
        setPID(itemToEdit.pid)

        setPname(itemToEdit.package_name)
        setPvalue(itemToEdit.package_value)
        setDailyProfit(itemToEdit.daily_profit)
        setMaxROI(itemToEdit.max_profit)
        setDuration(itemToEdit.duration)
        setMatchingLevel(itemToEdit.mathcing_level)
        dispatch(setEditpackagesidebar(true))
...
const handleSubmit = () => {
    ...
    const data = {
      pid,  pname,pvalue, dailyProfit, maxROI, duration, matchingLevel
    }
...
return (
    <>
      ...
      dark:bg-gray-800 ${!editpackagesidebar && "translate-x-full"}`}
      >
   ...
   <div class="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700" >
      <div class="space-y-6">
          <h5 class="text-xl font-medium text-gray-900 dark:text-white">Edit Existing Pakage</h5>
          <div>
